...
describe('Transaction Endpoints', () => {
    ...
    beforeAll(async () => {
        ...
        transactionData = {
            project_onchain_id: testProjectId1,
            USDC_amount: 5000,
            transaction_type: 'Investment',
            transaction_hash: '0x80486b40840a7d798af971844e6618a3cff5917150f8d50bebefe76bcc2ca0f6',
            platform_fee: 250,
        };
    });

    describe('POST /api/transactions/record', () => {
        it('should not record transaction with invalid body', async () => {
        ...
        it('should record a valid transaction', async () => {
            ...
            expect(res.body.data).toHaveProperty('transaction_hash', transactionData.transaction_hash);
        });

        it('should prevent duplicate transaction recording', async () => {
            const res = await request(app)
                .post('/api/transactions/record')
                .set('Authorization', `Bearer ${investorToken}`)
                .send(transactionData);
        ...
        it('should prevent transaction recording from DB error', async () => {
            const spy = jest.spyOn(transactionModel, 'getTransactionByTxHash').mockImplementation(() => { {
            ...
            const res = await request(app)
                ...
                .send({
                    ...transactionData,
                    transaction_hash: '0x6831345b75eb22b88bba6bab4b4b7ad98f752b14370e5e33ed8b9662731ec5fc'
                });
    ...
    describe('GET /api/transactions/my', () => {
        it('should get transactions for the authenticated investor', async () => {
            const res = await request(app)
        ...
        it('should prevent DB error when fetching user transactions', async () => {
            const spy = jest.spyOn(transactionModel, 'getTransactionsByUserId').mockImplementation(() => { {
                throw new Error('Database error');
            }});
            const res = await request(app)
                .get('/api/transactions/my')
    ...
    describe('GET /api/transactions/project/:projectId', () => {
        it('should prevent invalid data access by non-operator', async () => {
            const res = await request(app)
                .get(`/api/transactions/project/${"00000000-0000-0000-0000-00000"}`)
                .set('Authorization', `Bearer ${investorToken}`);
            expect(res.statusCode).toEqual(400);
            expect(res.body).toHaveProperty('message');
            expect(res.body.message).toMatch("must be");
        });

        it('should get transactions for a specific project by operator', async () => {
            const res = await request(app)
                .get(`/api/transactions/project/${testProjectId1}`)
        ...
        it('should prevent DB error when operator fetching project transactions', async () => {
            const spy = jest.spyOn(transactionModel, 'getTransactionsByProjectId').mockImplementation(() => { {
                throw new Error('Database error');
            }});
            const res = await request(app)
                .get(`/api/transactions/project/${testProjectId1}`)
    ...
    describe('GET /api/transactions/dividends', () => {
        it('should prevent access from who are not Platform Operator', async () => {
            const res = await request(app)
                .get('/api/transactions/dividends')
                .set('Authorization', `Bearer ${investorToken}`);
            expect(res.statusCode).toEqual(403);
            expect(res.body).toHaveProperty('message');
            expect(res.body.message).toMatch("Platform Operator role required");
        });

        it('should get all dividend transactions', async () => {
            const res = await request(app)
                .get('/api/transactions/dividends')
                .set('Authorization', `Bearer ${operatorToken}`);
            expect(res.statusCode).toEqual(200);
        ...
        it('should prevent DB error when operator fetching dividend transactions', async () => {
            const spy = jest.spyOn(transactionModel, 'getTransactionsByType').mockImplementation(() => { {
                throw new Error('Database error');
            }});
            const res = await request(app)
                .get('/api/transactions/dividends')
