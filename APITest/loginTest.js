...
const OUTPUT_FILE = path.join(OUTPUT_DIR, 'loginResponse.json');

const runLoginTest = async () => {
    try {
        console.log(`🚀 Starting login test for wallet: ${MOCK_WALLET.address}`);

        console.log("\n[1/3] Requesting nonce from the server...");
        const nonceResponse = await axios.get(`${API_BASE_URL}/auth/nonce/${MOCK_WALLET.address}`);
        ...
        console.log("\n[2/3] Signing the nonce with the wallet's privateKey:...");
        const wallet = new ethers.Wallet(MOCK_WALLET.privateKey);
        const signature = await wallet.signMessage(nonce);
        console.log(`✅ Success! Signature generated: ${signature.substring(0, 50)}...`);


        console.log("\n[3/3] Verifying signature with the server to get JWT token...");
        ...
        if (!token) {
            throw new Error("JWT Token not received after verification.");
        }
        else {
            localStorage.setItem('authToken', token);
            // Now the user is logged in, and apiClient will use this token automatically
        }
        ...
        console.log(`✅ Success! Login data saved to: ${OUTPUT_FILE}`);

    } catch (error) {
        console.error("\n❌ --- TEST FAILED --- ❌");
        if (error.response) {
            // Error from the API (e.g., 400, 404, 500)
            console.error(`Status: ${error.response.status}`);
            console.error(`Message: ${error.response.data.message}`);
            console.error("Data:", error.response.data);
        } else {
            // Other errors (e.g., network issue, coding error in the script)
